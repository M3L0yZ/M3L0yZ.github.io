const chapters = {
    accueil: {
        ...
        mute: true,
        bouton: [
            {titre: "Départ", destination: "debut"}
        ],
    },
    debut: {
        titre: "Il fait noir",
...
if(localStorage.getItem("mute") === "true") {
    muteOnOff.classList.replace('toggle-on', 'toggle-off');
}
...
muteOnOff.addEventListener("click", () =>{
    if(muteOnOff.classList.contains('toggle-on')) {
        muteOnOff.classList.replace('toggle-on', 'toggle-off');
        pluie.pause();
        localStorage.setItem("mute", "true");
        console.log("MUTE = ON")
    } else {
        muteOnOff.classList.replace('toggle-off', 'toggle-on');
        pluie.play();
        localStorage.setItem("mute", "false");
...
function goToChapter(chapter) {
    ...
    if(chapters[chapter].image) jeuImage.src = chapters[chapter].image;
    if(chapters[chapter].video) jeuVideo.src = chapters[chapter].video;

    if (!chapters[chapter].mute && localStorage.getItem("mute") == "false") {
        pluie.play();
    } else {
        pluie.pause();
    }
    ...
    if (chapters[chapter].image == null) {
        ...
        jeuVideo.play();
        jeuVideo.volume = 0;
    } else if (chapters[chapter].video == null) {
    ...
    if (chapter == "fin" && localStorage.getItem("twist", twist)) {
        console.log("Moment de vérité")
    } else if (chapter == "fin") {
        goToChapter("mensonge");
